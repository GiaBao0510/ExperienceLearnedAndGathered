## **0. Kiểm tra và cài đặt Redis trên Docker:**

Đầu tiên sẽ **cài đặt image** cho Redis:
```bash
docker pull redis
```

Nếu nghi ngờ image Redis chưa cài đặt, bạn có thể kiểm tra qua câu lệnh sau:
```shell
docker images redis
```

- Kết quả:
```shell
docker images redis
REPOSITORY   TAG       IMAGE ID       CREATED       SIZE
redis        latest    77e6be7eb290   2 weeks ago   139MB
```

Tiếp đến chúng ta sẽ ***cài đặt Container cho redis*** trên, thì bước cài đặt này sẽ thiết lập mật khẩu  (Ví dụ pwd là: "abc12345"). Và thêm thiết lập là nó sẽ tự động khởi động chỉ khi bật docker lên.

```shell
docker run -d --name my-redis-server -p 6379:6379 --restart unless-stopped redis redis-server --requirepass "MyRedis_12345"
```

Giải thích:
- `-d`:
	- Viết tắt của **detached**
	- Container chạy **ngầm dưới nền**
	- Không chiếm terminal của bạn

- `--name redis-server`
	-  Đặt **tên container**
	- Giúp dễ quản lý (start, stop, logs, exec…)
	-  Nếu không đặt tên → Docker tự sinh tên ngẫu nhiên (khó nhớ)

- `-p 6379:6379`:
	- Map cổng
	- Nhờ vậy bạn có thể kết nối Redis từ: localhost, ứng dụng .NET, Redis CLI,...

- `--restart unless-stopped`:
	- Cấu hình tự khởi động container

|Giá trị|Ý nghĩa|
|---|---|
|`no`|Không tự khởi động (mặc định)|
|`always`|Luôn tự khởi động, kể cả khi bạn stop thủ công|
|`unless-stopped`|**Tự khởi động khi Docker restart**, trừ khi bạn stop thủ công|


Để kết nối với **container redis (đã có container có sẳn và redis này cần xác thực mật khẩu)** thông quan `docker` thì dùng lệnh như sau. Ví dụ ta có thông tin container redis như sau:

```shell
CONTAINER ID   IMAGE          COMMAND                  CREATED       STATUS         PORTS                               NAMES
fc6b05b35fdb   redis:latest   "docker-entrypoint.s…"   10 days ago   Up 4 minutes   0.0.0.0:6379->6379/tcp              my_redis_server
```

Và đây là câu lệnh kết nối đến container redis:
```shell
docker exec -it my-redis-server redis-cli -a "Your_PWD"
```

---
## **1. Redis key**:
- Sử dụng các lệnh để thao tác ,quản lý các key trong redis:

| Command                 | Ý nghĩa                                               |
| ----------------------- | ----------------------------------------------------- |
| **EXISTS Key**          | Kiểm tra xem khóa có tồn tại không                    |
| **DEL key**             | Xóa khóa nếu tồn tại                                  |
| **EXPIRE key n**        | Đặt thời gian hết hạn cho key sau n giây              |
| **KEY patten**          | tìm các key theo patten                               |
| **TTL Key**             | Lấy thời gian sống của key (giây)                     |
| **PERSIST key**         | Xóa thời gian hết hạn cho key                         |
| **RENAMENX key newkey** | đổi tên khóa sang newkey dựa trên tên khóa đã tồn tại |
| **TYPE key**            | Lấy kiểu dữ liệu được lưu trữ bởi key                 |


---
## **2. Redis string**:
- Thao tác với các values kiểu string trong redis:

>Ví dụ:
```
127.0.0.1:6379> set myname giabao
OK
127.0.0.1:6379> get myname
"giabao"
```

| Command                     | Ý nghĩa                                                                                      |
| --------------------------- | -------------------------------------------------------------------------------------------- |
| **SET key value**           | đặt giá trị cho khóa nào đó                                                                  |
| **GET key**                 | lấy giá trị tại khóa cụ thể                                                                  |
| **GETRANGE key start end**  | Lấy giá trị của key bắt đầu từ start và kết thúc tại end                                     |
| **GETSET key value**        | Lấy giá trị cũ và đặt lại giá trị mới                                                        |
| **MGET key1 key2 ...**      | Lấy giá trị trên nhiều khóa nào đó                                                           |
| **SETEX key seconds value** | Đặt giá trị và thời gian hết hạn cho key                                                     |
| **SETNX key value**         | Đặt giá trị cho khóa nếu khóa chưa tồn tại                                                   |
| **RENAMENX key newkey**     | Lấy tên khóa hiện tại và đặt lại tên cho khóa đó. (Lưu ý tên khóa cần đặt phải chưa tồn tại) |
| **STRLEN Key**              | Lấy độ dài giá trị tại khóa                                                                  |
| **INCR key**                | Tăng giá trị lữu của khóa (số nguyên) 1 đơn vị                                               |
| **INCR key n**              | Tăng giá trị lữu của khóa (số nguyên) n đơn vị                                               |
| **DECR key**                | Giảm giá trị lưu trữ của khóa (số nguyên) 1 đơn vị                                           |
| **DECR key n**              | Giảm giá trị lưu trữ của khóa (số nguyên) n đơn vị                                           |

---
## **3. Redis hash**:
- được sử dụng để quản lý các key/value .Trong đó value có giá trị là hash.  Thường thì dùng hash để lưu trữ các object.

| Command                                                      | Ý nghĩa                                             |
| ------------------------------------------------------------ | --------------------------------------------------- |
| **HSET key field val**                                       | Đặt giá trị cho một trường cụ thể tại khóa nào đó   |
| **HGET key field**                                           | Lấy giá trị tại một trường của khóa nào đó          |
| **HMGET key field1 field2**                                  | Lấy giá trị tại các field của khóa nào đó           |
| **HGETAL**                                                   | Lấy tất cả giá trị theo từng trường của khóa cụ thể |
| **HDEL key field1 fiel**                                     | Xóa các trường tại khóa cụ thể                      |
| **HEXISTS key**                                              | Kiểm tra trường của khóa có tồn tại không           |
| **HINCRBY key**                                              | Tăng giá trị của field (Số nguyên) n đơn vị         |
| **HDECRBY ke**                                               | Giảm giá trị của field (Số nguyên) n đơn vị         |
| **HDECRBYFLOAT k**                                           | Giảm giá trị của field (Số thực)                    |
| **HVALS key**                                                | lấy tất cả các value của hash tại khóa              |
| **HLEN key**                                                 | Lấy số lượng trường tại khóa                        |
| **HMSET key field1 value1 field1 value1  ield2 value2 ....** | Đặt giá trị theo từng fiel trong khóa               |
*Ví dụ:*
```
127.0.0.1:6379> HSET user:1 name "john" age 30
OK
127.0.0.1:6379> HGET user:1 name
john
```

---
## **4.Redis list:**
- Thường được sử dụng để lưu các key/value trong đó thì value có kiểu là list. List được sử dụng phổ biến và list có 2 kiểu thường dùng là stack và queue.

> Ví dụ
```
127.0.0.1:6379> lpush TaiKhoan user1
(integer) 1
127.0.0.1:6379> lpush TaiKhoan user2
(integer) 2
127.0.0.1:6379> lpush TaiKhoan user3
(integer) 3
127.0.0.1:6379> lrange TaiKhoan 0 10
1) "user3"
2) "user2"
3) "user1"
```

| Command                             | Ý nghĩa                                                                         |
| ----------------------------------- | ------------------------------------------------------------------------------- |
| **LINDEX key index**                | lấy giá trị từ danh sách tại vị trí nào đó                                      |
| **LLEN key**                        | Lấy độ dài của danh sách                                                        |
| **LPOP key**                        | lấy phần tử ở đầu danh sách                                                     |
| **LPUSH key value1 value2 ...**     | Lấy các giá trị vào đầu danh sách                                               |
| **LRANGE key start end**            | Lấy các giá trị trong danh sách bắt đầu từ index start và kết thúc từ index end |
| **LSE ket index value**             | Đặt giá trị tại index cụ thể trong danh sách                                    |
| **RPOP key**                        | Lấy giá trị ở cuối danh sách                                                    |
| **RPUSH key value1 value2 ...**     | Thêm các giá trị lần lượt vào cuối danh sách                                    |
| **LINSERT key AFTER value1 value2** | Thêm phần tử value 2 vào trong sau phần tử value1                               |

---
## **5. Redis set:**
- Đây là một lập hợp dùng để lưu các **value** có giá trị duy nhất và không trùng lặp với nhau

_Ví dụ:_
```
127.0.0.1:6379> sadd server gachien
(integer) 1
127.0.0.1:6379> sadd server gvr
(integer) 1
127.0.0.1:6379> sadd server gfamily
(integer) 1
127.0.0.1:6379> smembers server
1) "gachien"
2) "gvr"
3) "gfamily"
```

| Command                    | Ý nghĩa                                                                   |
| -------------------------- | ------------------------------------------------------------------------- |
| **SADD key value1 value2 ...** | Thêm lần lượt các giá trị  vào trong tập hợp                              |
| **SCARD key**                  | Lấy số lượng phần tử trong tập hợp                                        |
| **SMEMBERS key**               | Lấy các phần tử trong tập hợp                                             |
| **SPOP key**                   | Xóa bỏ ngẫu nhiên một phần tử trong tập hợp ,rồi trả về giá trị đã bị xóa |

---
## **6. Redis sorted set**
- Dùng để quản lý key/value, trong đó value có giá trị mà một sorted set (tập hợp sắp xếp thep điểm/ ưu tiên từ thấp đến cao). Các giá trị trong sorted set là duy nhất.

_Ví dụ:_
```
127.0.0.1:6379> zadd todoi 1 dcod
(integer) 1
127.0.0.1:6379> zadd todoi 2 otc
(integer) 1
127.0.0.1:6379> zadd todoi 3 gfamily
(integer) 1
127.0.0.1:6379> zadd todoi 4 gvr
(integer) 1
127.0.0.1:6379> zrange todoi 0 10 withscores
1) "dcod"
2) "1"
3) "otc"
4) "2"
5) "gfamily"
6) "3"
7) "gvr"
8) "4"
```

| Command                                       | Ý nghĩa                                                                                                        |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| **ZADD key score1 value1 score2 value2 ....** | Thêm lần lượt các giá trị vào trong sorted set với độ ưu tiên dựa trên core                                    |
| **SCARD key**                                 | Lấy số lượng phần tử trong sorted set                                                                          |
| **ZRANGE key start end**                      | Lấy các phần tử trong tập hợp bắt đầu từ start và kết thúc bằng end                                            |
| **ZRANGE key start end WITHSCORS**            | Lấy các phần tử trong tập hợp bắt đầu từ start và kết thúc bằng end và kèm theo giá trị scrore trên từng value |
| **ZSCORE key member**                         | Lấy giá trị score của member                                                                                   |
| **ZRANK key member**                          | Lấy vị trí của member trong sorted set                                                                         |
| **ZCOUNT key score1 score2**                  | Đếm số member có score tương ứng trong đoạn score1 đến score2                                                  |

---
## **7. Redis Transaction :**
- Redis Transaction cho phép thực hiện nhóm các câu lệnh theo thứ tự cho đến hoàn thành câu lệnh cuối thì nó mới thực việc cập nhật dựa trên các câu lệnh trước đó.  Redis Transaction này bắt đầu bằng MULTI và kết thúc bằng EXEC

_Ví dụ:_
```
127.0.0.1:6379> multi
OK
127.0.0.1:6379(TX)> set fullname PhamGiaBao
QUEUED
127.0.0.1:6379(TX)> get fullname
QUEUED
127.0.0.1:6379(TX)> incr visitors
QUEUED
127.0.0.1:6379(TX)> exec
1) OK
2) "PhamGiaBao"
3) (integer) 1
```

| Command | Ý nghĩa                                |
| ------- | -------------------------------------- |
| **MULTI**   | Đánh dấu bắt đầu khối lệnh transaction |
| **EXEC**    | Thực hiện khối lệnh                    |
