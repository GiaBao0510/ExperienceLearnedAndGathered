...
![](https://www.computersciencebytes.com/wp-content/uploads/2017/05/ADO_Disconnected.png)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/0*eIEf7BPpdjat8vlD.jpg)
...
![](https://www.ezzylearning.net/wp-content/uploads/2020/07/dataset.gif) 
...
![](https://dotnettutorials.net/wp-content/uploads/2021/03/word-image-48.png?ezimgfmt=ng:webp/ngcb8)

---
### **6. SqlCommand object là gì?**

- Trong **ADO.NET**, đối tượng **SqlCommand** là một lớp được sử dụng để thực thi các câu lệnh SQL trong **CSDL SQL Server**. Nó là một phần trong thư viện **ADO.NET** và cung cấp các phương thức để thực hiện các truy vấn, cập nhật, xóa và chèn dữ liệu vào cơ sở dữ liệu. **SqlCommand** hỗ trợ các phương thức như **ExecuteNonQuery**, **ExecuteScalar** và **ExecuteReader** để thực hiện các loại truy vấn khác nhau.
- Lớp **SqlCommand** cho phép tạo ra các đối tượng mà từ đó có thể thực hiện các lệnh SQL tương tác với **MS SQL Server** như các mệnh đề UPDATE, INSERT,...
...
![](https://yinyangit.wordpress.com/wp-content/uploads/2011/08/ado-net-sqlcommand-object.png?)
...
![DataRelation](https://www.akadia.com/img/dotnet_dataview_manager.gif)
...
- Bằng cách sử dụng ==DataView==, có thể cho phép hiển thị dữ liệu trong một bảng với các thứ tự sắp xếp khác nhau và có thể lọc dữ liệu theo trạng thái hoặc dựa trên biểu thức bộ lọc. ==DataView== cung cấp một view động của dữ liệu, trong đó nội dung, thứ tự và các thành viên phản ánh các thay đổi trong ==DataTable== cơ bản khi xảy ra.
...
Sử dụng **DataView** trong các tình huống sau:
- **Khi cần tạo các chế độ xem dữ liệu khác nhau từ cùng một bảng dữ liệu:** Ví dụ, bạn muốn hiển thị dữ liệu được sắp xếp theo các cột khác nhau hoặc theo các tiêu chí lọc khác nhau trong các điều khiển giao diện người dùng khác nhau. 
- **Khi cần lọc hoặc sắp xếp dữ liệu mà không thay đổi dữ liệu gốc trong DataTable:** DataView cho phép bạn áp dụng tiêu chí lọc và sắp xếp mà không làm thay đổi dữ liệu gốc
- **Trong các ứng dụng ràng buộc dữ liệu:** DataView hỗ trợ ràng buộc dữ liệu động, cho phép các điều khiển giao diện người dùng tự động cập nhật để phản ánh các thay đổi trong dữ liệu
- **Khi cần quản lý cách dữ liệu hiển thị cho người dùng cuối:** Ví dụ, bạn có thể muốn chỉ hiển thị một phần của dữ liệu dựa trên quyền truy cập của người dùng hoặc các tiêu chí khác.
...
---
## 10.  **Sự khác biệt cơ bản giữa ADo.NET và Entity Framework là gì?**

**ADO.NET Entity Framework** là một **ORM (object-relational mapping)** tạo ra một mô hình đối tượng trừu tượng cao hơn trên các thành phần **ADO.NET**. **ADO.NET** là một lớp gần hơn so với cơ sở dữ liệu (bảng dữ liệu, bộ dữ liệu, .v.v). Lợi ích chính và duy nhất của **EF** là nó tự động tạo mã cho mô hình (lớp trung gian), lớp truy cập dữ liệu **(Data Access Layer)** và mã ánh xạ **(mapping code)**, giúp giảm thiểu đi rất nhiều thời gian phát triển. ***Ví dụ:***

**ADO.NET:**
```csharp
DataTable table = adoDs.Table[0];
for(int i = 0; i < table.Row.Count; i++){
	DataRow row = table.Rows[i];

	//Get the values of the fields
	string CustomerName = (string)row["Customername"];
	string CustomerCode = (string)row["CustomerCode"];
}
```

**EF:**
```csharp
foreach(Customer objCust in obj Customer){}
```

---
## 11. **SqlCommandBuilder trong ADO.NET là gì?**

![](https://www.sharpencode.com/Images/ADO/ADO14.PNG)

SqlCommandBuilder là một lớp trong **ADO.NET** được sử dụng để tạo ra các câu lệnh SQL cho việc cập nhật dữ liệu trong **CSDL** khi sử dụng **SqlDataAdapter**. Lớp này giúp đơn giản hóa quá trình tạo các câu lệnh **SQL** cho các thao tác `Insert`, `Update`, `Delete` dựa trên của một DataTable trong **DataSet SqlCommandBuilder** tự động tạo ra các câu lệnh **SQL** cần thiết khi bạn gọi phương thức **Update** của **SqlDataAdapter**.

Các tính năng chính của SqlCommandBuilder:
- **Tự động tạo câu lệnh SQL:** **SqlCommandBuilder** tự động tạo các câu lệnh `Insert`, `Update`, `Delete` dựa trên **SelectCommand** của **SqlDataAdapter**.
- **Đơn giản hóa quá trình cập nhật dữ liệu:** Giảm thiểu việc phải viết thủ công các câu lệnh SQL cho các thao tác cơ bản.
- **Phương thức DeriveParamenters:** Lấy thông tin tham số từ **stored procedure**.
- Phương thức RefreshSchema: Cập nhật **schema** của **SqlCommandBuilder** để phản ánh các thay đổi trong **DataTable**.

Lợi ích của **SqlCommandBuilder**:
- **Tiết kiệm thời gian:** tự động tạo ra các câu lệnh SQL giúp tiết kiệm thời gian và công sức.
- ***GIảm thiểu lỗi:*** GIảm thiểu khả năng xảy ra lỗi do viết sai câu lệnh SQL thủ công.
- **Dễ sửu dụng:** Cung cấp một cách đơn giản và hiệu quả để quản lý các thao tác cập nhật dữ liệu.
- **SqlCommandBuilder** là một công cụ hữu ích trng **ADO.NET** giúp đơn giản hóa quá trình cập nhật dữ liệu trong **CSDL**, đặc biệt khi làm việc với **SqlDataAdapter** và **DataSet**.
